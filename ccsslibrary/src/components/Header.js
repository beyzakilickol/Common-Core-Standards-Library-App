...
class Header extends Component{
  ...
  render(){
          ...
          if(this.props.usertype == 'regular'){
            withRegularUser = <div className="cartWrapper"><div className="dropdown-menu userDropdown" role="menu"><Link to="/sellerregistration" className="dropdown-item">Become a seller</Link><Link to="/sellerpurchases" className="dropdown-item">My Purchases</Link><Link to="/sellerwishlist" className="dropdown-item">My Wish List</Link><Link to="/" onClick={this.logout} className="dropdown-item" role="presentation" href="#">Logout</Link></div><span className="fa-stack fa-x has-badge cartImg" data-count=""><Link to='/viewcart'><i className="fa fa-shopping-cart number">{this.props.cartcount}</i></Link>
            </span></div>
          } else {
